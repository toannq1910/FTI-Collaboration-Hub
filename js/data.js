...
{id:'nd13-2023',title:'Nghị định 13/2023/NĐ-CP - Bảo vệ dữ liệu cá nhân',issuer:'Chính phủ',url:'https://vanban.chinhphu.vn/?docid=207759&pageid=27160',summary:'Nguồn chính cho cơ chế đồng ý, xử lý dữ liệu cá nhân, quyền chủ thể dữ liệu, đánh giá tác động và chuyển dữ liệu cá nhân ra nước ngoài.'},
{id:'nd91-2020',title:'Nghị định 91/2020/NĐ-CP - Chống tin nhắn rác, thư điện tử rác, cuộc gọi rác',issuer:'Chính phủ',url:'https://datafiles.chinhphu.vn/cpp/files/vbpq/2020/08/91.signed.pdf',summary:'Nguồn chính cho cuộc gọi quảng cáo, thời gian gọi, danh sách không quảng cáo, cơ chế từ chối và nghĩa vụ nhận diện cuộc gọi.'}
];

export const complianceCards=[
{id:'telecom-numbering',title:'Quy Định Đầu Số Viễn Thông',icon:'🚨',type:'danger',sources:['nd91-2020'],bullets:['Đầu số 1800/1900, số cố định và SIP trunk cần kiểm tra điều kiện đăng ký theo nhà mạng/cơ quan quản lý trước khi triển khai.','Số 1800/1900 cần gắn đúng chủ thể doanh nghiệp, mục đích sử dụng và kịch bản chăm sóc khách hàng.','Số cố định 028/024 cần khớp thông tin địa điểm, hợp đồng và hồ sơ doanh nghiệp.','BYOC/SIP trunk chỉ dùng với nhà mạng hoặc đối tác viễn thông có điều kiện cung cấp phù hợp tại Việt Nam.','Outbound từ contact center cần log đầy đủ số gọi ra, thời điểm, agent/campaign và trạng thái xử lý.','Trước khi publish cho khách hàng, cần kiểm tra lại hồ sơ đầu số với nhà mạng triển khai.']},
{id:'call-recording-security',title:'Luật Ghi Âm & Bảo Mật Cuộc Gọi',icon:'⚠️',type:'warn',sources:['nd13-2023'],bullets:['Thông báo cho khách hàng khi cuộc gọi được ghi âm, ví dụ qua IVR announcement hoặc thông báo đầu cuộc gọi.','Ghi âm có thể chứa dữ liệu cá nhân nên cần cơ chế đồng ý, mục đích xử lý rõ ràng và phân quyền truy cập.','File ghi âm phải lưu trữ an toàn, có audit log khi nghe/tải/xóa và không chia sẻ cho bên thứ ba ngoài phạm vi được phép.','Thời hạn lưu trữ cần cấu hình theo ngành, hợp đồng và chính sách nội bộ thay vì dùng một mốc cố định cho mọi khách hàng.','Nếu ghi âm hoặc transcript được đẩy qua API/webhook, cần kiểm soát dữ liệu cá nhân và điểm lưu trữ cuối.']},
{id:'personal-data-protection',title:'Bảo Vệ Dữ Liệu Cá Nhân (NĐ 13/2023)',icon:'🔒',type:'danger',sources:['nd13-2023'],bullets:['Nghị định 13/2023/NĐ-CP là nguồn chính về bảo vệ dữ liệu cá nhân tại Việt Nam.','Khi thu thập số điện thoại, email, lịch sử cuộc gọi, ghi âm hoặc ticket, cần có mục đích xử lý và cơ chế đồng ý phù hợp.','Cần phân loại dữ liệu cá nhân, dữ liệu cá nhân nhạy cảm và giới hạn người được truy cập trong CMS/CRM/contact center.','Chuyển dữ liệu cá nhân ra nước ngoài cần rà soát hồ sơ đánh giá tác động và nghĩa vụ thông báo/cung cấp theo quy định.','Cần có đầu mối hoặc bộ phận phụ trách bảo vệ dữ liệu cá nhân, xử lý yêu cầu truy cập, chỉnh sửa, xóa hoặc rút lại đồng ý.','Không hardcode hoặc log lộ PII trong API, webhook, transcript, screen-pop hoặc file export.']},
{id:'outbound-advertising',title:'Quy Định Cuộc Gọi Quảng Cáo / Outbound',icon:'📞',type:'warn',sources:['nd91-2020'],bullets:['Cuộc gọi quảng cáo phải tuân thủ Nghị định 91/2020/NĐ-CP về chống cuộc gọi rác.','Không gọi quảng cáo ngoài khung giờ được phép; riêng cuộc gọi quảng cáo cần kiểm tra mốc 8:00-17:00 theo Nghị định 91.','Phải nhận diện rõ doanh nghiệp/nhãn hàng và mục đích cuộc gọi ở phần đầu kịch bản.','Tôn trọng Danh sách không quảng cáo và cơ chế khách hàng từ chối nhận cuộc gọi quảng cáo.','Campaign outbound cần lưu log: nguồn data, consent/opt-in, thời điểm gọi, số lần gọi, kết quả và opt-out.','Predictive/progressive dialer cần giới hạn tần suất để tránh spam và khiếu nại.']},
{id:'finance-special',title:'Ngành Tài Chính — Yêu Cầu Đặc Biệt',icon:'🏦',type:'info',sources:['nd13-2023'],bullets:['Ngân hàng, bảo hiểm, chứng khoán và fintech cần legal review riêng trước khi bật ghi âm, campaign hoặc tích hợp dữ liệu khách hàng.','Cuộc gọi tư vấn, xác nhận giao dịch hoặc khiếu nại có thể cần ghi âm, lưu trữ và audit trail theo quy định ngành.','Không lưu PAN/CVV hoặc dữ liệu thẻ nhạy cảm trong CRM, ticket, transcript hoặc ghi chú agent.','OTP/voice OTP/SMS OTP cần được thiết kế theo nguyên tắc tối thiểu dữ liệu và không ghi log mã xác thực thô.','API log cần đủ để truy vết nhưng phải masking dữ liệu nhạy cảm trước khi hiển thị cho agent/supervisor.','Thời hạn lưu trữ cần cấu hình theo chính sách pháp lý, kế toán và hợp đồng từng khách hàng.']},
{id:'secure-deployment',title:'Best Practices — Triển Khai An Toàn',icon:'✅',type:'success',sources:['nd13-2023'],bullets:['Dùng TLS 1.2+ cho API, webhook, portal quản trị và kết nối tích hợp liên quan.','Quản lý API key/token theo vòng đời: tạo, phân quyền, rotate, thu hồi và audit.','Không hardcode credential trong source code, tài liệu demo, slide hoặc file cấu hình public.','Rate limiting, retry policy và circuit breaker cần có cho webhook/API tích hợp CRM/ERP.','Audit log các thao tác quan trọng: đăng nhập, xem/tải ghi âm, export dữ liệu, xóa dữ liệu, thay đổi phân quyền.','Masking số điện thoại, email, định danh cá nhân trong log, dashboard và dữ liệu mẫu demo.']}];
...
{icon:'🛡️',title:'Compliance VN',desc:'Checklist đầu số, ghi âm, dữ liệu cá nhân, outbound, tài chính và bảo mật triển khai.',sources:['nd13-2023','nd91-2020']},
