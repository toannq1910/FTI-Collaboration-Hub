...
import{stats,solutions,vendorGroups,compareRows,complianceCards,complianceSources,resources,partnerProductCatalog}from'./data.js';
import{getProductAssets,assetObjectUrl,formatBytes,assetPut,generatePresentationPages,assetGet}from'./cms/cms-assets.js?v=20260701-6';
...
const complianceSourceById=Object.fromEntries(complianceSources.map(source=>[source.id,source]));
function renderComplianceSourceLinks(sourceIds=[]){
  return (sourceIds||[]).map(id=>{
    const source=complianceSourceById[id];
    if(!source)return '';
    const url=source.url||'#resources';
    const isExternal=/^https?:\/\//i.test(url);
    return `<a class="source-chip" href="${safeHtml(url)}" ${isExternal?'target="_blank" rel="noopener"':''}>
      <b>${safeHtml(source.title)}</b>
      <span>${safeHtml(source.issuer||'Nguồn tham chiếu')}</span>
    </a>`;
  }).filter(Boolean).join('');
}
function renderCompliance(){
  const cards=getComplianceCards();
  return `<section class="hero"><span class="eyebrow">🛡️ Vietnam Compliance</span><h2>Quy Định & Tuân Thủ Tại Việt Nam</h2><p>Các yêu cầu pháp lý, kỹ thuật khi triển khai UC/CCaaS tại thị trường Việt Nam.</p></section><div class="alert">⚠️ Lưu ý quan trọng: Tài liệu này cung cấp thông tin chung về quy định tại Việt Nam. Doanh nghiệp cần tham vấn pháp lý chuyên nghiệp trước khi triển khai.</div><div class="section-head"><div><h2>Danh sách bài viết tuân thủ</h2><p>Nội dung được quản trị tập trung trong CMS Data để tránh xung đột dữ liệu.</p></div></div><div class="compliance-grid" style="margin-top:18px">${cards.map(c=>`<div class="compliance-card ${safeHtml(c.type||'')}"><h3>${safeHtml(c.icon||'')} ${safeHtml(c.title||'')}</h3><ul class="list">${(c.bullets||[]).map(b=>`<li>${safeHtml(b)}</li>`).join('')}</ul>${c.sources?.length?`<div class="card-source-block"><strong>Dẫn chứng</strong><div class="source-chip-list">${renderComplianceSourceLinks(c.sources)}</div></div>`:''}</div>`).join('')}</div>`;
}
function renderResources(){
  return `<div class="section-head"><div><h2>Nguồn tài liệu & bộ công cụ</h2><p>Dành cho khách hàng, presales và đội triển khai.</p></div></div><div class="resource-grid">${resources.map(r=>`<div class="resource-card"><div class="icon">${safeHtml(r.icon||'')}</div><div><h4>${safeHtml(r.title||'')}</h4><p>${safeHtml(r.desc||'')}</p>${r.sources?.length?`<div class="resource-source-panel"><b>Nguồn Compliance VN</b><div class="source-chip-list">${renderComplianceSourceLinks(r.sources)}</div></div>`:''}</div></div>`).join('')}</div>`;
}
