...
const OFFICIAL_ASSETS = [
  {
    ...
    thumbnail: 'assets/presentation/oncallcx-pages/page-01.jpg',
    official: true
  },
  {
    ...
    thumbnail: 'assets/presentation/oncallcx-ucaas-pages/page-01.jpg',
    official: true
  },
  {
...
function productOptions(data, selected = 'oncallcx'){
  ...
  assetProductLabels = new Map(Array.from(byId.values()).map(p => [p.id, p.title || p.id]));
  return Array.from(byId.values()).map(p => `<option value="${esc(p.id)}" ${p.id === selected ? 'selected' : ''}>${esc(p.title || p.id)}</option>`).join('');
}

function productLabel(id = 'oncallcx'){
  return assetProductLabels.get(id) || ASSET_PRODUCT_SHORTCUTS.find(p => p.id === id)?.title || id || 'OnCallCX';
}

function assetThumbnail(record){
  const image = record.thumbnail || record.pages?.[0]?.image || '';
  if(image){
    return `<img src="${esc(image)}" alt="${esc(record.title || record.fileName || assetTypeLabel(record.type))}">`;
  }
  return `<div class="asset-thumb-icon">${assetIcon(record.type)}</div>`;
}
...
export function renderAssetManager(data = null, description = ''){
  ...
  <div class="asset-modal" id="assetPreviewModal" hidden>
    <button class="asset-modal-backdrop" type="button" data-asset-modal-close aria-label="Đóng preview"></button>
    <section class="asset-modal-panel" role="dialog" aria-modal="true" aria-labelledby="assetPreviewTitle">
      <div class="asset-modal-head">
        <div>
          <strong id="assetPreviewTitle">Preview</strong>
          <small id="assetPreviewMeta">Chọn một asset để xem trước</small>
        </div>
        <button class="btn btn-soft" type="button" data-asset-modal-close>Đóng</button>
      </div>
      <div id="assetPreviewBody" class="asset-modal-body"></div>
    </section>
  </div>`;
}
...
export async function renderAssetList(){
  ...
  root.innerHTML = assets.length ? assets.map(a => `<article class="asset-card" data-asset-view="${a.id}" tabindex="0" role="button" aria-label="Xem ${esc(a.title || a.fileName || 'asset')}">
    <div class="asset-thumb">${assetThumbnail(a)}</div>
    <div class="asset-card-body">
      <div class="asset-card-badges">
        <span>${esc(assetTypeLabel(a.type))}</span>
        ${a.official ? '<span class="official">Mặc định</span>' : ''}
      </div>
      <h3>${esc(a.title || a.fileName || 'Untitled asset')}</h3>
      <p>${esc(a.fileName || '')}</p>
      <small>${esc(productLabel(a.product || 'oncallcx'))} · ${formatBytes(a.size)} · ${new Date(a.createdAt).toLocaleString('vi-VN')}</small>
...
async function openAssetModal(id){
  ...
  const modal = $('#assetPreviewModal');
  if(!modal) return;
  $('#assetPreviewTitle').textContent = rec.title || rec.fileName || 'Preview';
  $('#assetPreviewMeta').textContent = `${assetTypeLabel(rec.type)} · ${productLabel(rec.product || 'oncallcx')} · ${formatBytes(rec.size)}`;
...
function clearReplacementSelection(){
  ...
  const uploadBtn = $('#assetUploadBtn');

  if(replaceId) replaceId.value = '';
  if(replacementSlot) replacementSlot.value = '';
  if(notice){
    notice.hidden = true;
    notice.innerHTML = '';
  }
  if(uploadBtn) uploadBtn.textContent = 'Upload & Link Asset';
}

function showReplacementSelection(record){
  const notice = $('#assetReplaceNotice');
  const uploadBtn = $('#assetUploadBtn');
  if(notice){
    notice.hidden = false;
    notice.innerHTML = `
      <b>Đang thay thế: ${esc(record.title || record.fileName)}</b>
      <small>Chọn file mới rồi bấm Upload thay thế. Bản upload mới sẽ ưu tiên hiển thị; khi xóa bản upload mới, file mặc định sẽ tự hiện lại.</small>
      <button type="button" class="btn btn-soft asset-replace-cancel" id="assetReplaceCancel">Hủy thay thế</button>
    `;
    $('#assetReplaceCancel')?.addEventListener('click', clearReplacementSelection);
  }
  if(uploadBtn) uploadBtn.textContent = 'Upload thay thế';
}

function bindAssetActions(){
  $$('[data-asset-view]').forEach(el => {
    el.onclick = async (event) => {
      event.preventDefault();
      event.stopPropagation();
      await openAssetModal(el.dataset.assetView);
    };
    el.onkeydown = async (event) => {
  ...
  $$('[data-asset-download]').forEach(btn => btn.onclick = async (event) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  $$('[data-asset-delete]').forEach(btn => btn.onclick = async (event) => {
    event.preventDefault();
    event.stopPropagation();
  ...
  $$('[data-asset-replace]').forEach(btn => btn.onclick = async (event) => {
    ...
    if(notice) showReplacementSelection(rec);
...
export function bindAssetManager(data = null){
  ...
  $$('[data-asset-modal-close]').forEach(btn => btn.onclick = closeAssetModal);
  if(!assetModalEscapeBound){
    assetModalEscapeBound = true;
